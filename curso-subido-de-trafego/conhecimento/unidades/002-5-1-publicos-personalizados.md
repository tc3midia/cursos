...
confianca: alta
versao: 2
nota: "Correção manual de F02: acrescentadas as contagens da demonstração, preservando seu caráter temporário."
```
Na demonstração, aparecem cinco opções em “suas fontes” e nove em “fontes da Meta”.
Essas contagens são temporárias: tanto a quantidade quanto os tipos disponíveis podem mudar.
Entenda o princípio de criação para aplicá-lo também às fontes que forem acrescentadas.
...
### U:2adad5b1c731d4ab:005 — Retenção de visitantes: divergência entre fala e interface
```yaml
tipo: regua
plataforma: [meta]
tema: publicos
...
confianca: media
versao: 2
nota: "Correção manual de F01 e F15: preservada a divergência entre o intervalo informado pelo professor e a mensagem de interface que ele lê; confiança reduzida e perecibilidade marcada. A janela não esclarece se exatamente 180 dias é aceito."
```
O professor informa retenção de 1 até 180 dias para o público de todos os visitantes do site.
Ao testar um valor maior, ele lê uma mensagem da interface exigindo valor menor que 180.
A fala apresenta 180 dias como extremo do intervalo, mas a mensagem relatada o exclui; a fonte não resolve essa divergência.
...
confianca: alta
versao: 2
nota: "Correção manual de F03: acrescentada a possibilidade de selecionar visitantes por tempo gasto em páginas específicas; marcada a perecibilidade dessa opção de configuração."
```
...
Na configuração demonstrada, essa seleção pode considerar o site inteiro ou páginas específicas, reunindo o percentil que passou mais tempo nas páginas escolhidas.
...
confianca: alta
versao: 2
nota: "Correção manual de F04: incorporados os três exemplos de frequência, sem transformá-los em recomendação de uso."
```
...
Para uma página específica, o professor demonstra selecionar quem a visitou mais de duas vezes, exatamente duas vezes ou menos de duas vezes.
...
confianca: alta
versao: 2
nota: "Correção manual de F05: acrescentada a descrição opcional e marcada a perecibilidade por mencionar esse campo da interface."
```
...
4. Se desejar, acrescente uma descrição ao público; esse campo é opcional.
...
tarefas: [montar-publicos-personalizados]
fonte: fala
faixa: "00:19:29–00:21:35"
perecivel: false
confianca: alta
versao: 2
nota: "Correção manual de F07: acrescentada a criação de todos os períodos e ampliada a faixa para incluir essa orientação. A lista de períodos retoma a sequência ensinada para site, cuja criação é reaplicada pelo professor ao catálogo."
```
...
Para cada uma dessas condições, deixe criadas as variações de 1, 3, 7, 14, 30, 60, 90 e 180 dias, retomando os períodos apresentados para site.
...
### U:2adad5b1c731d4ab:028 — Organize contatos na mesma coluna; cabeçalho é opcional
```yaml
tipo: regra
plataforma: [meta]
tema: publicos
...
confianca: alta
versao: 2
nota: "Correção manual de F08: explicitada a disposição dos contatos na mesma coluna, conforme a demonstração, preservando a orientação de cabeçalho opcional."
```
Disponha os contatos da lista na mesma coluna, conforme orienta o professor na demonstração.
Essa coluna pode ter ou não um cabeçalho identificando e-mail ou contato; segundo a explicação, essa identificação não é necessária para o Meta reconhecer a lista.
...
confianca: alta
versao: 2
nota: "Correção manual de F09: registrada a inclusão opcional do ano e a preferência do professor por omiti-lo."
```
Pré-condição: arquivo de lista de clientes ou cadastrados pronto para envio.
1. Comece por “lista M”, usando M para “mutável”.
2. Inclua dia e mês da atualização, como “27/03”; o ano pode ser incluído ou omitido, e o professor prefere omiti-lo.
...
tarefas: [montar-publicos-personalizados]
fonte: fala
faixa: "00:37:46–00:40:36"
perecivel: true
confianca: alta
versao: 2
nota: "Correção manual de F10: acrescentada a retenção inicial de um dia e ampliada a faixa para cobrir sua configuração. O professor chama a prática de rataria e demonstra criação repetida por cliques rápidos."
```
Pré-condição: vídeos já selecionados e público de vídeo configurado.
1. Defina inicialmente a retenção em 1 dia, antes de criar as cópias.
2. Clique rapidamente várias vezes em “criar público” para gerar cópias.
3. Entre em cada cópia e edite a retenção e o nome.
4. Atualize o público após mudar a retenção; mudar apenas o nome não basta.
5. Apague os públicos de exemplo que não devem ser usados.
...
confianca: alta
versao: 2
nota: "Correção manual de F11: acrescentada a seleção de um formulário específico ou de todos os formulários, com marcação de perecibilidade dessa opção."
```
...
Na seleção demonstrada, o público pode considerar um formulário específico ou todos os formulários da página escolhida.
...
tarefas: [montar-publicos-personalizados]
fonte: fala
faixa: "00:44:03–00:46:56"
perecivel: true
confianca: alta
versao: 2
nota: "Correção manual de F13 e F14: registrado o relato histórico de disponibilidade do público de seguidores, acrescentada a recomendação de todos os períodos e ampliada a faixa para cobri-la."
```
Na demonstração do perfil do Instagram, há públicos de todos, seguidores, visitou o perfil, salvou, enviou mensagem e engajou com publicações.
O professor relata que a opção de seguidores antes não estava disponível para todos e que, na ocasião da aula, essa disponibilidade havia sido ampliada; o relato não garante disponibilidade posterior.
Seguidores não permite definir número de dias; os demais usam retenção de até 365 dias.
Para os tipos que permitem retenção, crie variações em todos os períodos apresentados na aula, especialmente para o tipo “todos”.
...
### U:2adad5b1c731d4ab:051 — Nomeie público de aplicativo com tipo, nome e período
```yaml
tipo: procedimento
plataforma: [meta]
tema: nomenclatura
tarefas: [nomear-campanhas, montar-publicos-personalizados]
fonte: fala
faixa: "00:17:40–00:18:18"
perecivel: false
confianca: alta
versao: 1
nota: "Inclusão manual de F06: criado procedimento de nomenclatura sem alterar as unidades 023 e 024. O nome próprio do aplicativo usado como exemplo não foi necessário para preservar o padrão."
```
Pré-condição: negócio com aplicativo e público de atividade no aplicativo configurado.
1. Comece o nome por “APP”.
2. Acrescente o nome do aplicativo.
3. Indique a condição do público, como “abriu”.
4. Termine com o período de retenção, como “30 dias”.

### U:2adad5b1c731d4ab:052 — Nomeie público de formulário com estado e retenção
```yaml
tipo: procedimento
plataforma: [meta]
tema: nomenclatura
...
confianca: alta
versao: 1
nota: "Inclusão manual de F12: criado procedimento de nomenclatura, preservando os estados descritos na fonte e sem alterar a régua da unidade 045."
```
Pré-condição: público de formulário configurado para um formulário específico ou para todos os formulários.
1. Comece o nome por “formulário”.
2. Indique o estado: “abriu”, “abriu mas não enviou” ou “abriu e enviou”.
3. Acrescente o nome do formulário selecionado ou “todos”, conforme a seleção.
4. Termine com a retenção em dias, respeitando o máximo de 90 dias.

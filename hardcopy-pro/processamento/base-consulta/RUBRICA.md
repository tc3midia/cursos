# Rubrica — unidades da base Hardcopy Pro

Versão 1 · 20/09/2026. Mesma rubrica do Formato Criativo de Conteúdo, que adaptou a v2 do Curso Subido; a seção final traz o que é próprio deste curso. Sem nota numérica. Cinco vetos, aplicados com evidência e impacto. Revisão proporcional: corrigir o que está errado, sem reextrair módulo por contagem de reprovações.
...
Aplicações dos cinco vetos ao que o Hardcopy Pro tem de particular. Não criam veto novo.

- **Copy de exemplo tratada como fato** (fidelidade, material). Percentual, depoimento, preço ou alegação dita dentro de VSL ou anúncio fictício escrito na aula, apresentada como `regua`, `regra`, `conceito` ou dado de mercado. O esperado é `exemplo` com a condição de copy de exemplo, ou ficar de fora.
- **Passo de tela inventado** (fidelidade, sem-lastro material). Procedimento com passo, menu, valor ou botão que a fala não nomeia. Em aula de tela, poucas unidades e um `limite` dizendo o que ficou só na imagem é o resultado correto, não omissão.
- **Resíduo de silêncio ou laço de repetição usado como fonte** (fidelidade, sem-lastro material).
- **Termo deformado promovido a nome** (fidelidade). Grafia deformada da tabela do FORMATO usada como se fosse outro conceito ou outra ferramenta, ou ferramenta identificada por hipótese sem `nota`. Duas grafias do mesmo termo não são dois conceitos.
- **Ressalva do professor suprimida** (fidelidade, material quando inverte a recomendação): "eu não usaria", "não indico", "foi só didático, não vou testar".
- **Técnica que contorna política de plataforma ou direito de terceiros sem o aviso em `nota`** (aplicabilidade, material): quem consulta precisa saber que a decisão de aplicar é dele. Sem `perecivel: true` também reprova por perecibilidade.
- **Não é omissão:** testemunho pessoal, motivação, construção de autoridade, oferta de outro produto além do `limite` da aula, conteúdo que só existe na tela.
- **Não é falha:** unidade que mantém entre aspas um termo não identificado, com `nota` e `confianca: baixa`.
